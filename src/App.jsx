...
async function generateSignedPdf(templatePdfUrl, fields, formData, pdfDims, scale) {
  ...
  for (const field of fields) {
    ...
    if (field.type === "signature" || field.type === "initials") {
      ...
      page.drawText(txt, { x:pdfX+2, y:pdfY+4, size:Math.min(11,fH*0.6), font, color:rgb(0.06,0.01,0.26), maxWidth:fW-4 });
    }
  }
  ...
  return (
    <div>
      {field.type==="signature"||field.type==="initials" ? (
        ...
        <input autoFocus className="form-input" type={field.type==="date"?"date":field.type==="email"?"email":field.type==="phone"?"tel":"text"} value={val} onChange={e=>setVal(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submit()} /></>
      )}
            ...
            return (
              <div key={f.id} className="sign-field-overlay" style={{left:f.x,top:f.y,width:f.w,height:f.h}} onClick={()=>setActiveField(f)}>
                {(f.type==="signature"||f.type==="initials")&&filled
                  ...
                  : <div className={`sign-field-box ${filled?"filled":""}`} style={isErr?{borderColor:"#e05555",background:"rgba(224,85,85,0.1)",color:"#c03030"}:{}}>
                      {filled?(f.type==="checkbox"?"✓":formData[f.id]):`${FIELD_TYPES.find(t=>t.type===f.type)?.icon} ${f.label}${f.required?" *":""}`}
                    </div>}
              </div>
            );
          })}
...
function TemplateBuilder({ template, onSave, onCancel }) {
  ...
  const handleUpload = async (e) => {
    ...
    if (!error) { const {data:{publicUrl}} = supabase.storage.from("template-pdfs").getPublicUrl(fname); setPdfUrl(publicUrl); }
  ...
  const onDrop = (e) => {
    ...
    setFields(p=>[...p,{id,type:fieldType,label:def.label,x:Math.max(0,e.clientX-rect.left-def.w/2),y:Math.max(0,e.clientY-rect.top-def.h/2),w:def.w,h:def.h,required:true}]);
    setSelectedId(id);
  };
    ...
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 62px)"}}>
      ...
      <div className="builder-layout" style={{flex:1}}>
        <div className="builder-sidebar">
          ...
          {fields.length>0&&<><div className="builder-sidebar-title">Placed</div>
            {fields.map(f=>(
              <div key={f.id} className="field-chip" style={{cursor:"pointer",...(selectedId===f.id?{borderColor:"#FF47E2",background:"rgba(255,71,226,0.15)"}:{})}} onClick={()=>setSelectedId(f.id)}>
