...
function SignatureInput({ height = 140, onChange, value, isInitials = false, fontSize = null }) {
  ...
  useEffect(() => {
    ...
    if (isInitials) { ctx.textAlign = "center"; ctx.fillText(typedText, 250, 80); }
    else { ctx.textAlign = "left"; ctx.fillText(typedText, 16, 80); }
    // Crop canvas to just the inked pixels so the image doesn't have extra whitespace
    const pixels = ctx.getImageData(0, 0, 500, 120).data;
    let minX = 500, maxX = 0, minY = 120, maxY = 0;
    for (let y = 0; y < 120; y++) {
      for (let x = 0; x < 500; x++) {
        const alpha = pixels[(y * 500 + x) * 4 + 3];
    ...
    const cropW = Math.min(500, maxX + pad) - cropX, cropH = Math.min(160, maxY + pad) - cropY;
