...
.sign-field-sig{width:100%;height:100%;object-fit:contain;object-position:left center;padding:2px;display:block}
...
// ════════════════════════════════════════════════
function SignatureInput({ height = 140, onChange, value, isInitials = false, fontSize = null }) {
  // Scale admin pt size up for the 500px-wide canvas
  const canvasFontSize = fontSize ? Math.max(12, Math.min(fontSize * 3.5, 120)) : (isInitials ? 54 : 46);
  ...
  useEffect(() => {
    ...
    // Measure text first so canvas is always wide enough
    const cH = 160;
    c.width = 1200; c.height = cH;
    let ctx2 = c.getContext("2d");
    ctx2.font = `${canvasFontSize}px ${f.css}`;
    const measured = ctx2.measureText(typedText);
    const textW = Math.ceil(measured.width) + 40; // extra room for italic overhang
    c.width = Math.max(textW, 200); c.height = cH;
    const ctx = c.getContext("2d");
    ctx.clearRect(0, 0, c.width, cH);
    ctx.font = `${canvasFontSize}px ${f.css}`;
    ctx.fillStyle = "#100142";
    ctx.textBaseline = "middle";
    if (isInitials) { ctx.textAlign = "center"; ctx.fillText(typedText, c.width / 2, cH / 2); }
    else { ctx.textAlign = "left"; ctx.fillText(typedText, 16, cH / 2); }
    // Crop to actual inked pixels
    const pixels = ctx.getImageData(0, 0, c.width, cH).data;
    let minX = c.width, maxX = 0, minY = cH, maxY = 0;
    for (let y = 0; y < cH; y++) {
      for (let x = 0; x < c.width; x++) {
        const alpha = pixels[(y * c.width + x) * 4 + 3];
        if (alpha > 10) { minX=Math.min(minX,x); maxX=Math.max(maxX,x); minY=Math.min(minY,y); maxY=Math.max(maxY,y); }
      }
    }
    if (maxX < minX) { onChange && onChange(null); return; }
    const pad = 16;
    const cropX = Math.max(0, minX - pad), cropY = Math.max(0, minY - pad);
    const cropW = Math.min(c.width, maxX + pad) - cropX, cropH = Math.min(cH, maxY + pad) - cropY;
    ...
    onChange && onChange(cropped.toDataURL());
  }, [typedText, selectedFont, mode, canvasFontSize]);
  ...
  const ed = () => {
    ...
    const pad = 6;
    const cropX = Math.max(0, minX-pad), cropY = Math.max(0, minY-pad);
    const cropW = Math.min(c.width, maxX+pad) - cropX, cropH = Math.min(c.height, maxY+pad) - cropY;
    const cropped = document.createElement("canvas");
    cropped.width = cropW; cropped.height = cropH;
    cropped.getContext("2d").drawImage(c, cropX, cropY, cropW, cropH, 0, 0, cropW, cropH);
    onChange && onChange(cropped.toDataURL());
    ...
    <div className="sig-container">
      ...
      {mode === "draw" ? (
        ...
        <div className="sig-type-area">
          ...
          {fontSize && <p style={{fontSize:11,color:"var(--text2)",marginTop:10}}>Will render at {fontSize}px in the PDF</p>}
          <canvas ref={exportRef} style={{display:"none"}} width={500} height={160} />
        </div>
      )}
    </div>
  );
  ...
  for (const field of fields) {
    ...
    if (field.type === "signature" || field.type === "initials") {
      if (typeof val === "string" && val.startsWith("data:image")) {
        ...
        // Scale to fit within the field box while preserving aspect ratio
        const imgSize = img.size();
        const imgAspect = imgSize.width / imgSize.height;
        const boxAspect = fW / fH;
        let drawW, drawH;
        if (imgAspect > boxAspect) {
          drawW = fW; drawH = fW / imgAspect;
        } else {
          drawH = fH; drawW = fH * imgAspect;
        }
        // Vertically centre within the field, left-align horizontally
        const drawY = pdfY + (fH - drawH) / 2;
        page.drawImage(img, { x: pdfX, y: drawY, width: drawW, height: drawH });
      }
  ...
  return (
    <div>
      {field.type==="signature"||field.type==="initials" ? (
        ...
        <SignatureInput height={field.type==="initials"?100:150} value={val} onChange={setVal} isInitials={field.type==="initials"} fontSize={field.fontSize||null} /></>
      ) : field.type==="checkbox" ? (
            ...
            return (
              <div key={f.id} className="sign-field-overlay" style={{left:f.x,top:f.y,width:f.w,height:f.h}} onClick={()=>setActiveField(f)}>
                {(f.type==="signature"||f.type==="initials")&&filled
                  ? <img src={formData[f.id]} className="sign-field-sig" alt="sig" style={{objectPosition:"left center"}} />
    ...
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 62px)"}}>
      <div style={{background:"#100142",borderBottom:"1px solid rgba(255,71,226,0.3)",padding:"10px 18px",display:"flex",alignItems:"center",gap:12,flexShrink:0}}>
        ...
        {sel && <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          {(sel.type==="text"||sel.type==="full_name"||sel.type==="date"||sel.type==="email"||sel.type==="phone"||sel.type==="signature"||sel.type==="initials") && (
...
function FamilyPortal({ token }) {
  ...
  if (error) return <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",background:"#100142"}}><div style={{textAlign:"center"}}><img src="/logo.png" alt="Logo" style={{height:350}} /><p style={{color:"#FF47E2",fontSize:17,fontFamily:"'DM Sans',sans-serif"}}>{error}</p></div></div>;
